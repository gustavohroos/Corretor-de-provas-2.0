...
{
	int i, cod, valido = 0, posicaoErr, posicaoCerta;
	float mediaAcertos;
	
	mediaAcertos = posicaoErr = posicaoCerta = cod = 0;
	do{
		gabarito *celula = Lista_Gabaritos;
		system("clear");
		printf("\n\n\t --------------------------------\n");
	...
	posicaoCerta = Questao_Mais_Acertada(cod);
	posicaoErr = Questao_Mais_Errada(cod);
	mediaAcertos = Media_Acertos(cod);
	
	printf("\n\n\t ----Estatísticas da prova %d---\n", cod);
	printf("\t| Questão mais acertada: %d        |\n", posicaoCerta + 1);
	printf("\t ---------------------------------\n");
	printf("\t| Questão mais errada: %d          |\n", posicaoErr + 1) ;
	printf("\t ---------------------------------\n");
	printf("\t| Média de acertos: %.2f          |\n", mediaAcertos);
	printf("\t ---------------------------------\n");

	printf("\n\n\t\t\tNOME\t\t\t   MATRÍCULA  | ACERTOS\n\n");

	for(i = 1, ponteiro_aluno = Lista_Alunos; ponteiro_aluno->prox != NULL; ponteiro_aluno = ponteiro_aluno->prox)
	{
...
{
  int i, totalQuest, k, maximo, posicao;
	aluno *celula;
